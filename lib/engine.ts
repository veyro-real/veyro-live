...
export async function state(session?:string){const base={mode:mode(),network:mode()==='testnet'?'Solana testnet':'Local rehearsal',xConfigured:!!process.env.X_BEARER_TOKEN,configured:mode()==='rehearsal'||!!process.env.VEYRO_DEPLOYMENT_FILE,cloudAudit:!!(process.env.SUPABASE_URL&&process.env.SUPABASE_SERVICE_ROLE_KEY),mainnetEnabled:false,quoteLabel:'TEST-USD',outputLabel:'TEST-MEME'};if(!session)return {...base,policy:null,attempts:[]};try{return {...base,policy:await currentPolicy(session),attempts:await attempts(session)};}catch(e){return {...base,policy:null,attempts:await attempts(session),error:(e as Error).message};}}
