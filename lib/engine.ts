...
import {Rpc,keyFromSecret,generateKey,exportKey,decodePolicy,swapIx,revokeIx,errorReason,createPolicyIx,policyAddress,systemCreate,initializeToken,mintTo,TOKEN,PROGRAM_ID} from '@veyro/sdk';
...
export type DemoPolicy={active:boolean;expiresAt:number;maxAmount:string;totalLimit:string;spent:string;recipient:string;nonce:string;address:string;agent:string};
const defaultPolicy=():DemoPolicy=>({active:true,expiresAt:Math.floor(Date.now()/1000)+3600,maxAmount:'100000000',totalLimit:'150000000',spent:'0',recipient:'demo-owner',nonce:'0',address:'rehearsal-policy',agent:'rehearsal-agent'});
...
export async function act(session:string,body:any,operator:boolean):Promise<unknown>{return exclusive(async()=>{
 ...
 try{
  ...
  if(kind==='configure'){
   ...
   if(mode()==='rehearsal'){const p={...defaultPolicy(),maxAmount:intent.budgetMicros,totalLimit:total.budgetMicros,expiresAt:Math.floor(Date.now()/1000+hours*3600),address:'rehearsal-'+randomUUID()};saveState('policy:'+session,p);a.policy=p;a.reason='REHEARSAL_POLICY_CREATED';a.status='REHEARSED';}
   else {
    const c=deployment(),rpc=rpcFor(c);await rpc.assertTestCluster();const current=await currentPolicy(session);if(current.active)throw Error('REVOKE_CURRENT_POLICY_FIRST');
    ...
    const tx=await rpc.transaction(owner,[admin,vault],[createPolicyIx({owner:owner.publicKey,agent:agent.publicKey,executor:c.executor,recipient:owner.publicKey,pool:c.pool,maxAmount:BigInt(intent.budgetMicros),totalLimit:BigInt(total.budgetMicros),expiresAt:BigInt(Math.floor(Date.now()/1000+hours*3600)),minRate:990n},c.programId),systemCreate(owner.publicKey,vault.publicKey,BigInt(rent),165n,TOKEN),initializeToken(vault.publicKey,c.quoteMint,policy),mintTo(c.quoteMint,vault.publicKey,admin.publicKey,BigInt(total.budgetMicros))]);
   ...
   if(kind==='research'){a.reason='RESEARCH_COMPLETE';a.status='COMPLETE';recordAttempt(session,a);return {attempt:publicAttempt(a),...await state(session)};}
   ...
   const recipient=['unauthorized','compromised'].includes(scenario)?'attacker':p.recipient;a.request={...a.request as object,amount:amount.toString(),recipient};
   if(mode()==='rehearsal'){const reason=previewPolicy(p,{amount:amount.toString(),recipient});a.reason=reason;a.decision=reason==='POLICY_SATISFIED'?'ALLOW':'DENY';a.status='REHEARSED';if(a.decision==='ALLOW')saveState('policy:'+session,{...p,spent:(BigInt(p.spent)+amount).toString(),nonce:(BigInt(p.nonce)+1n).toString()});}
   else{
    ...
    const c=deployment(),rpc=rpcFor(c),agent=key(c.agentKey||'agent'),executor=key('executor');const tx=await rpc.transaction(executor,[agent],[swapIx({...c,recipient:recipient==='attacker'?c.attacker:c.recipient},amount,amount*990n,BigInt(p.nonce),c.programId)]);
    const sim=await rpc.simulate(tx.bytes);a.simulationSlot=sim.context.slot;a.reason=errorReason(sim.value.logs,sim.value.err);a.decision=sim.value.err?'DENY':'ALLOW';a.status=sim.value.err?'DENIED':'EVALUATED';recordAttempt(session,a);
