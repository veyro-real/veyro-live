...
export async function research(live:boolean):Promise<{candidates:ResearchCandidate[];source:string;cached:boolean}>{
 ...
 const token=await credential('xBearerToken');if(!token)throw Error('X_NOT_CONFIGURED');
 const cached=await getState<{at:number;candidates:ResearchCandidate[]}>('x:cache');if(cached && Date.now()-cached.at<300_000)return {candidates:cached.candidates,source:'X recent search',cached:true};
 const key='x:requests:'+new Date().toISOString().slice(0,10);const count=await getState<number>(key)||0;const cap=Math.min(50,Math.max(1,Number(process.env.X_MAX_REQUESTS_PER_DAY||10)));if(!Number.isFinite(cap)||count>=cap)throw Error('X_DAILY_REQUEST_CAP');
 await saveState(key,count+1); // Count attempted requests even when upstream fails; no automatic retries.
 ...
 const rpcUrl=await credential('mainnetRpcUrl')||'https://api.mainnet-beta.solana.com';const rpc=new Rpc(rpcUrl);
 ...
 if(!candidates.length)throw Error('NO_VERIFIED_SOLANA_CANDIDATE');await saveState('x:cache',{at:Date.now(),candidates});return {candidates,source:'X recent search',cached:false};
}
