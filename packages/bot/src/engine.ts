...
export async function deployment(){const saved=await getState<any>('runtime');if(saved)return saved;const file=process.env.VEYRO_DEPLOYMENT_FILE;if(!file)throw Error('TESTNET_NOT_CONFIGURED');return JSON.parse(readFileSync(file,'utf8'));}
...
export async function currentPolicy(session:string):Promise<DemoPolicy>{if(mode()==='rehearsal'){let p=await getState<DemoPolicy>('policy:'+session);if(!p){p=defaultPolicy();await saveState('policy:'+session,p);}return p;}const c=await deployment(),rpc=rpcFor(c);await rpc.assertTestCluster();const a=await rpc.account(c.policy);if(!a||a.owner!==c.programId)throw Error('POLICY_NOT_FOUND');const p=decodePolicy(a.data);return {...serial(p),expiresAt:Number(p.expiresAt),address:c.policy};}
export async function state(session?:string){const creds=await credentialStatus().catch(()=>({xBearerToken:!!process.env.X_BEARER_TOKEN,jupiterApiKey:!!process.env.JUPITER_API_KEY,mainnetRpcUrl:!!process.env.SOLANA_MAINNET_READ_RPC_URL}));const base={mode:mode(),network:mode()==='testnet'?'Solana testnet':'Local rehearsal',xConfigured:creds.xBearerToken,configured:mode()==='rehearsal'||!!process.env.VEYRO_DEPLOYMENT_FILE,cloudAudit:!!(process.env.SUPABASE_URL&&process.env.SUPABASE_SERVICE_ROLE_KEY),mainnetEnabled:false,quoteLabel:'TEST-USD',outputLabel:'TEST-MEME',credentials:creds};if(!session)return {...base,policy:null,attempts:[]};try{return {...base,policy:await currentPolicy(session),attempts:await attempts(session)};}catch(e){return {...base,policy:null,attempts:await attempts(session),error:(e as Error).message};}}
export async function reconcile(session:string){if(mode()!=='testnet')return;const c=await deployment(),rpc=rpcFor(c);await rpc.assertTestCluster();for(const a of await attempts(session,true)){if(!a.candidateSignature||!['SIGNED','SUBMITTED','UNKNOWN','CONFIRMED'].includes(a.status))continue;const status=(await rpc.call('getSignatureStatuses',[[a.candidateSignature],{searchTransactionHistory:true}])).value[0];if(status?.err){a.status='FAILED';a.reason=JSON.stringify(status.err);}else if(status?.confirmationStatus==='finalized'){a.status='FINALIZED';a.executedSignature=a.candidateSignature;if(a.pendingRuntime)await saveState('runtime',a.pendingRuntime);}else if(status){a.status='CONFIRMED';}else if(a.lastValidBlockHeight && await rpc.call<number>('getBlockHeight',[{commitment:'finalized'}])>a.lastValidBlockHeight){a.status='EXPIRED';a.reason='BLOCKHASH_EXPIRED_WITHOUT_OBSERVED_SETTLEMENT';}else a.status='UNKNOWN';await recordAttempt(session,a);}}
export async function act(session:string,body:any,operator:boolean):Promise<unknown>{return exclusive(async()=>{
 ...
 const existing=await claimRequest(session,idempotency);if(existing){if(existing==='IN_PROGRESS')throw Error('REQUEST_IN_PROGRESS_OR_RECOVERY_REQUIRED');return {attempt:(await attempts(session)).find(a=>a.id===existing),...await state(session)};}
 const a=await beginAttempt(session,kind,{intent:body.intent,budget:body.budget,scenario:body.scenario,liveResearch:body.liveResearch===true});await finishRequest(session,idempotency,a.id);
 try{
  if(mode()==='testnet'&&['configure','revoke','faucet'].includes(kind)&&!operator)throw Error('OWNER_ACTION_REQUIRED');
  if(kind==='reconcile'){await reconcile(session);a.decision='ALLOW';a.reason='RECONCILED';a.status='COMPLETE';await recordAttempt(session,a);return {attempt:publicAttempt(a),...await state(session)};}
  if(kind==='faucet'){if(mode()==='rehearsal')throw Error('FAUCET_REQUIRES_TESTNET');const c=await deployment(),rpc=rpcFor(c);await rpc.assertTestCluster();const rateKey='faucet:last';if(Date.now()-(await getState<number>(rateKey)||0)<60000)throw Error('FAUCET_COOLDOWN');await saveState(rateKey,Date.now());a.candidateSignature=await rpc.call<string>('requestAirdrop',[c.executor,500_000_000]);a.decision='ALLOW';a.status='SUBMITTED';a.reason='FAUCET_REQUESTED';await recordAttempt(session,a);return {attempt:publicAttempt(a),...await state(session)};}
  if(kind==='configure'){
   ...
   const c=mode()==='testnet'?await deployment():null;const allowedRecipients=list(body.allowedRecipients,c?[c.owner]:['demo-owner'],8);const allowedPrograms=list(body.allowedPrograms,[TOKEN],4);
   if(mode()==='rehearsal'){const p={...defaultPolicy(),maxAmount:intent.budgetMicros,totalLimit:total.budgetMicros,allowedRecipients,allowedPrograms,expiresAt:Math.floor(Date.now()/1000+hours*3600),address:'rehearsal-'+randomUUID()};await saveState('policy:'+session,p);a.agent=p.agent;a.policy=p;a.decision='ALLOW';a.reason='REHEARSAL_POLICY_CREATED';a.status='REHEARSED';}
   else {
    ...
    a.agent=agent.publicKey;a.policy={address:policy,owner:owner.publicKey,agent:agent.publicKey,allowedRecipients,allowedPrograms,maxAmount:intent.budgetMicros,totalLimit:total.budgetMicros,expiresAt:Math.floor(Date.now()/1000+hours*3600)};a.decision='ALLOW';a.candidateSignature=tx.signature;a.wire=Buffer.from(tx.bytes).toString('base64');a.lastValidBlockHeight=tx.lastValidBlockHeight;a.status='SIGNED';a.pendingRuntime={...c,policy,agent:agent.publicKey,agentKey:name,vault:vault.publicKey};await recordAttempt(session,a);await rpc.send(tx.bytes);await rpc.confirm(tx.signature);await saveState('runtime',a.pendingRuntime);a.executedSignature=tx.signature;a.status='FINALIZED';a.reason='POLICY_CREATED';
   }
  }else if(kind==='revoke'){
   const p=await currentPolicy(session);a.policy=p;a.agent=p.agent;
   if(mode()==='rehearsal'){await saveState('policy:'+session,{...p,active:false});a.decision='ALLOW';a.status='REHEARSED';a.reason='REVOKED';}
   else{const c=await deployment(),rpc=rpcFor(c),owner=key('owner');const tx=await rpc.transaction(owner,[],[revokeIx(owner.publicKey,c.policy,c.programId)]);a.decision='ALLOW';a.candidateSignature=tx.signature;a.wire=Buffer.from(tx.bytes).toString('base64');a.lastValidBlockHeight=tx.lastValidBlockHeight;a.status='SIGNED';await recordAttempt(session,a);await rpc.send(tx.bytes);await rpc.confirm(tx.signature);a.executedSignature=tx.signature;a.status='FINALIZED';a.reason='REVOKED';}
  }else if(kind==='run'||kind==='research'){
   const intent=parseIntent(String(body.intent||''),String(body.budget||''));a.request={...a.request as object,intent};await recordAttempt(session,a);
   if(body.liveResearch===true && mode()==='rehearsal' && !operator)throw Error('OPERATOR_REQUIRED_FOR_PAID_RESEARCH');
   const findings=await research(body.liveResearch===true);a.research=findings;const selected=pickCandidate(findings.candidates);a.selected=selected;a.testTokenMapping={researchMint:selected.mint,executedToken:'TEST-MEME',sameAsset:false};await recordAttempt(session,a);
   if(kind==='research'){a.decision='ALLOW';a.reason='RESEARCH_COMPLETE';a.status='COMPLETE';await recordAttempt(session,a);return {attempt:publicAttempt(a),...await state(session)};}
   ...
   if(mode()==='rehearsal'){const reason=previewPolicy(p,{amount:amount.toString(),recipient,program:TOKEN});a.reason=reason;a.decision=reason==='POLICY_SATISFIED'?'ALLOW':'DENY';a.status='REHEARSED';if(a.decision==='ALLOW')await saveState('policy:'+session,{...p,spent:(BigInt(p.spent)+amount).toString(),nonce:(BigInt(p.nonce)+1n).toString()});}
   else{
    await reconcile(session);if((await attempts(session,true)).some(x=>x.id!==a.id && ['SIGNED','SUBMITTED','UNKNOWN','CONFIRMED'].includes(x.status)))throw Error('PREVIOUS_TRANSACTION_UNRESOLVED');
    const c=await deployment(),rpc=rpcFor(c),agent=key(c.agentKey||'agent'),executor=key('executor');if(!deniedRecipient&&recipient!==c.owner)throw Error('RECIPIENT_ACCOUNT_UNAVAILABLE');const recipientTokenAccount=deniedRecipient?c.attacker:c.recipient;a.request={...a.request as object,recipientTokenAccount};const tx=await rpc.transaction(executor,[agent],[swapIx({...c,recipient:recipientTokenAccount},amount,amount*990n,BigInt(p.nonce),c.programId)]);
    const verdict=await evaluateTransaction(rpc,tx.bytes);a.simulationSlot=verdict.simulationSlot;a.reason=verdict.reason;a.decision=verdict.decision;a.status=verdict.decision==='DENY'?'DENIED':'EVALUATED';await recordAttempt(session,a);
    if(a.decision==='ALLOW'){a.candidateSignature=tx.signature;a.wire=Buffer.from(tx.bytes).toString('base64');a.lastValidBlockHeight=tx.lastValidBlockHeight;a.status='SIGNED';await recordAttempt(session,a);await rpc.send(tx.bytes);a.status='SUBMITTED';await recordAttempt(session,a);await rpc.confirm(tx.signature);a.executedSignature=tx.signature;a.status='FINALIZED';}
   }
 ...
 await recordAttempt(session,a);return {attempt:publicAttempt(a),...await state(session)};
 });}
